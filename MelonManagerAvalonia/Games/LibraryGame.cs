...
using MelonManagerAvalonia.Configs;
using MelonManagerAvalonia.Interfaces;
using MelonManagerAvalonia.Managers;
using MelonManagerAvalonia.MelonLoader;
using MelonManagerAvalonia.Melons;
using MelonManagerAvalonia.Tasks;
...
public class LibraryGame
    {
        ...
        private Melon VerifyMelon(string melonPath, bool isPlugin, bool newMelon = false)
        {
            ...
            if (!melon.CheckCompatibility(info))
            {
                ...
                if (newMelon)
                    //TODO CustomMessageBox.Error($"The {(isPlugin ? "plugin" : "mod")} '{melon.name}' is incompatible with {info.name}.");
                if (!invalidsList.Contains(melonPath))
                    invalidsList.Add(melonPath);
            ...
            if (melon.isPlugin != isPlugin)
            {
                ...
                if (newMelon)
                    //TODO CustomMessageBox.Error($"The melon you're trying to add is a {(isPlugin ? "mod" : "plugin")} and cannot be added to the {(isPlugin ? "plugins" : "mods")} list.");
                if (!invalidsList.Contains(melonPath))
                    invalidsList.Add(melonPath);
        ...
        public void AddMelon(string path, bool isPlugin)
        {
            ...
            {
                //TODO CustomMessageBox.Error($"A melon with the same file name '{mel}' already exists!");
